...
const apiImpl = {
  ...
  deleteCustomer: async (id:number) => { if (!isSupabaseConfigured) return expressApi.deleteCustomer(id); row(await sb().from('customers').update({deleted_at:new Date().toISOString()}).eq('id',id).is('deleted_at',null).select('id').single()); },
