...
export interface Appointment { id?: number; customer_id: number; customerName?: string; service_id: number | null; service_name?: string; service?: string; duration_minutes?: number; custom_service_name?: string | null; custom_service_duration?: number | null; custom_service_price?: number | null; start_time: string; end_time?: string; status: AppointmentStatus; price?: number; total_amount?: number; product_total?: number; products?: { product_id: number; quantity: number }[]; custom_items?: { name: string; amount: number }[]; surcharge_type?: 'none'|'percent'|'fixed'; surcharge_value?: number; notes?: string }
...
type AppointmentQueryRow = { id:number; customer_id:number; service_id:number|null; custom_service_name:string|null; custom_service_duration:number|null; custom_service_price:number|null; start_time:string; end_time:string; status:AppointmentStatus; total_amount:number; custom_items:unknown; note:string|null; customers:{name:string}|null; services:{name:string;duration_min:number;price:number}|null };
const mapAppointment = (value: AppointmentQueryRow): Appointment => ({ id:value.id, customer_id:value.customer_id, customerName:value.customers?.name, service_id:value.service_id, custom_service_name:value.custom_service_name, custom_service_duration:value.custom_service_duration, custom_service_price:value.custom_service_price, service_name:value.custom_service_name ?? value.services?.name, service:value.custom_service_name ?? value.services?.name, duration_minutes:value.custom_service_duration ?? value.services?.duration_min, price:value.custom_service_price ?? value.services?.price, total_amount:value.total_amount, custom_items:Array.isArray(value.custom_items) ? value.custom_items as {name:string;amount:number}[] : [], start_time:value.start_time, end_time:value.end_time, status:value.status, notes:value.note ?? '' });
const appointmentSelect = 'id,customer_id,service_id,custom_service_name,custom_service_duration,custom_service_price,start_time,end_time,status,total_amount,custom_items,note,customers(name),services(name,duration_min,price)';
...
const apiImpl = {
  ...
  updateAppointment: async (value: Appointment) => {
    ...
    row(await sb().rpc('update_appointment', { p_id:value.id, p_customer_id:value.customer_id, p_service_id:value.service_id, p_start_time:value.start_time, p_status:value.status, p_custom_items:value.custom_items ?? [], p_note:value.notes, p_custom_service_name:value.custom_service_name, p_custom_service_duration:value.custom_service_duration, p_custom_service_price:value.custom_service_price }));
  ...
  getServices: async () => isSupabaseConfigured ? row(await sb().from('services').select('*').is('deleted_at',null).order('name')).map(mapService) : expressApi.getServices(),
  ...
  deleteService: async (id:number) => { if (!isSupabaseConfigured) return expressApi.deleteService(id); row(await sb().from('services').update({ active:false, deleted_at:new Date().toISOString() }).eq('id',id).is('deleted_at',null).select('id').single()); },
  ...
  getCustomers: async () => isSupabaseConfigured ? row(await sb().from('customers').select('*').is('deleted_at', null).order('name')).map(mapCustomer) : expressApi.getCustomers(),
  ...
  updateCustomer: async (value:Customer) => { if (!isSupabaseConfigured) return expressApi.updateCustomer(value); if(value.id===undefined) throw new Error('Customer id is required.'); row(await sb().from('customers').update({name:value.name,phone:value.phone,email:value.email || null,note:value.notes}).eq('id',value.id).is('deleted_at',null).select('id').single()); },
  deleteCustomer: async (id:number) => { if (!isSupabaseConfigured) return expressApi.deleteCustomer(id); row(await sb().rpc('permanently_delete_customer',{p_customer_id:Number(id)})); },
  ...
  deleteProduct: async (id:number) => { if (!isSupabaseConfigured) return expressApi.deleteProduct(id); row(await sb().rpc('delete_product',{p_product_id:id})); },
