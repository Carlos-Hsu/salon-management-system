...
export type Database = {
  public: {
    Tables: {
      ...
      orders: { Row: { id:number; appointment_id:number; idempotency_key:string; status:'paid'|'refunded'; service_amount:number; product_amount:number; custom_amount:number; discount:number; total_amount:number; payment_method:string; custom_items:unknown[]; created_at:string }; Insert: never; Update: never; Relationships: [{ foreignKeyName:'orders_appointment_id_fkey'; columns:['appointment_id']; isOneToOne:true; referencedRelation:'appointments'; referencedColumns:['id'] }] };
