...
function NewBookingDialog({ open, onClose, customers, services, onCreate }: { open: boolean; onClose(): void; customers: Customer[]; services: Service[]; onCreate: Props['onCreate'] }) {
  ...
  const selectedServiceId = activeServices.some(service => service.id === serviceId) ? serviceId : activeServices[0]?.id ?? 0;
  useEffect(() => {
    if (!open) return;
    setCustomerId(current => customers.some(customer => customer.id === current) ? current : customers[0]?.id ?? 0);
    setServiceId(current => activeServices.some(service => service.id === current) ? current : activeServices[0]?.id ?? 0);
  }, [open, customers, activeServices]);
  const close = () => { setCustomItems([]); onClose(); };
  const save = async (event: React.FormEvent) => { event.preventDefault(); if (!selectedServiceId) { setMessage('目前沒有可預約的服務，請先到設定啟用服務。'); return; } if (customItems.some(item => !item.name.trim())) { setMessage('請填寫所有臨時項目名稱。'); return; } setSaving(true); setMessage(''); try { await onCreate({ customer_id: customerId || customers[0]?.id || 0, service_id: selectedServiceId, start_time: new Date(start).toISOString(), status: 'pending', custom_items: customItems.map(({ name, amount }) => ({ name: name.trim(), amount })) }); close(); } catch (error) { setMessage(appointmentMutationErrorMessage(error)); } finally { setSaving(false); } };
  return <Dialog.Root open={open} onOpenChange={value => { if (!value) close(); }}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[200] bg-black/60"/><Dialog.Content className="checkout-dialog fixed left-1/2 top-1/2 z-[201] w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 p-5"><div className="flex justify-between"><div><Dialog.Title className="text-2xl font-black">新增預約</Dialog.Title><Dialog.Description>服務清單會同步目前設定中已啟用的項目</Dialog.Description></div><Dialog.Close className="min-h-12 min-w-12"><X className="mx-auto"/></Dialog.Close></div><form className="mt-5 grid gap-3" onSubmit={save}><label>顧客<select required value={customerId || customers[0]?.id || ''} onChange={event => setCustomerId(Number(event.target.value))}>{customers.map(customer => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><label>服務<select required value={selectedServiceId || ''} onChange={event => setServiceId(Number(event.target.value))}>{activeServices.map(service => <option key={service.id} value={service.id}>{service.name} · {service.duration_minutes} 分</option>)}</select></label><label>開始<input required type="datetime-local" value={start} onChange={event => setStart(event.target.value)}/></label><CustomLineItemEditor items={customItems} onChange={setCustomItems}/>{message && <p className="settings-error" role="alert">{message}</p>}<button disabled={saving || !customers.length || !activeServices.length} className="min-h-12 rounded-xl border border-amber-400/40 bg-amber-500 font-semibold text-slate-950 transition-colors hover:bg-amber-400 disabled:border-slate-700 disabled:bg-slate-800 disabled:text-slate-500">{saving ? '儲存中…' : '建立待確認預約'}</button></form></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
