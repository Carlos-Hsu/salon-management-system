...
import { api, appointmentMutationErrorMessage, type Appointment, type AppointmentStatus, type Customer, type Product, type Service } from '../../api';
  ...
  return <Dialog.Root open={Boolean(appointment)} onOpenChange={open => { if (!open) onClose(); }}>
    <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[200] bg-black/60"/><Dialog.Content className="checkout-dialog fixed inset-x-0 bottom-0 z-[201] max-h-[92vh] overflow-y-auto rounded-t-2xl bg-white p-5 shadow-panel md:inset-y-0 md:left-auto md:right-0 md:w-[480px] md:rounded-none md:p-7">
      ...
      <div className="mt-5 space-y-3"><div className="flex justify-between"><span>服務費</span><strong>NT$ {(appointment?.price ?? 0).toLocaleString()}</strong></div>
        ...
        <CustomLineItemEditor items={customItems} onChange={setCustomItems}/><label className="block font-bold">折扣<input className="mt-1 w-full" type="number" min="0" value={discount} onChange={event => setDiscount(Math.max(0, Number(event.target.value)))}/></label><label className="block font-bold">付款方式<select className="mt-1 w-full" value={payment} onChange={event => setPayment(event.target.value)}><option value="cash">現金</option><option value="credit_card">信用卡</option><option value="line_pay">LINE Pay</option><option value="bank_transfer">轉帳</option></select></label><div className="flex justify-between border-t-2 pt-3 text-2xl font-black"><span>應收</span><span>NT$ {total.toLocaleString()}</span></div>
      </div>
...
function AppointmentDetailsDialog({ appointment, customers, services, onClose, onUpdate, onDelete, onCheckout }: {
  ...
  const [customServiceName, setCustomServiceName] = useState(''); const [customServiceDuration, setCustomServiceDuration] = useState(60); const [customServicePrice, setCustomServicePrice] = useState(0);
  ...
  useEffect(() => {
    ...
    setCustomerId(appointment.customer_id); setServiceId(appointment.service_id ?? -1);
    setCustomServiceName(appointment.custom_service_name ?? ''); setCustomServiceDuration(appointment.custom_service_duration ?? 60); setCustomServicePrice(appointment.custom_service_price ?? 0);
    setStart(localDateTime(appointment.start_time)); setStatus(appointment.status);
    setNotes(appointment.notes ?? ''); setMessage('');
  }, [appointment]);

  const draft = (): Appointment => ({ ...appointment!, customer_id: customerId, service_id: serviceId === -1 ? null : serviceId, custom_service_name:serviceId === -1 ? customServiceName.trim() : null, custom_service_duration:serviceId === -1 ? Math.round(customServiceDuration) : null, custom_service_price:serviceId === -1 ? Math.round(customServicePrice) : null, start_time: new Date(start).toISOString(), status, notes });
  const save = async (event: React.FormEvent) => {
    ...
    } catch (error) { setMessage(appointmentMutationErrorMessage(error)); }
    finally { setSaving(false); }
  };
  const remove = async () => {
  ...
  return <Dialog.Root open={Boolean(appointment)} onOpenChange={open => { if (!open) onClose(); }}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[180] bg-black/60"/><Dialog.Content className="checkout-dialog fixed left-1/2 top-1/2 z-[181] max-h-[92vh] w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto p-5">
    ...
    <form className="mt-5 grid gap-3" onSubmit={save}>
      ...
      <label>服務<select required value={serviceId} onChange={event => setServiceId(Number(event.target.value))}>{services.filter(service => service.active || service.id === serviceId).map(service => <option key={service.id} value={service.id}>{service.name} · {service.duration_minutes} 分</option>)}<option value={-1}>＋ 自訂本次服務</option></select></label>
      {serviceId === -1 && <div className="grid gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3"><label>自訂服務名稱<input required value={customServiceName} onChange={event => setCustomServiceName(event.target.value)}/></label><label>所需時間（分鐘）<input required type="number" min="1" step="5" value={customServiceDuration} onChange={event => setCustomServiceDuration(Number(event.target.value))}/></label><label>服務價位（NT$）<input required type="number" min="0" value={customServicePrice} onChange={event => setCustomServicePrice(Number(event.target.value))}/></label></div>}
...
function NewBookingDialog({ open, onClose, customers, services, onCreate }: { open: boolean; onClose(): void; customers: Customer[]; services: Service[]; onCreate: Props['onCreate'] }) {
  ...
  const isCustomService = serviceId === -1;
  const selectedServiceId = isCustomService ? -1 : activeServices.some(service => service.id === serviceId) ? serviceId : activeServices[0]?.id ?? -1;
  useEffect(() => {
    if (!open) return;
    setCustomerId(current => customers.some(customer => customer.id === current) ? current : customers[0]?.id ?? 0);
    setServiceId(current => current === -1 || activeServices.some(service => service.id === current) ? current : activeServices[0]?.id ?? -1);
  }, [open, customers, activeServices]);
  const close = () => { setCustomItems([]); setCustomServiceName(''); setCustomServiceDuration(60); setCustomServicePrice(0); onClose(); };
  const save = async (event: React.FormEvent) => { event.preventDefault(); if (isCustomService && (!customServiceName.trim() || customServiceDuration < 1 || customServicePrice < 0)) { setMessage('請填寫自訂服務名稱、至少 1 分鐘的時長與有效價位。'); return; } if (customItems.some(item => !item.name.trim())) { setMessage('請填寫所有臨時項目名稱。'); return; } setSaving(true); setMessage(''); try { await onCreate({ customer_id: customerId || customers[0]?.id || 0, service_id: isCustomService ? null : selectedServiceId, custom_service_name:isCustomService ? customServiceName.trim() : null, custom_service_duration:isCustomService ? Math.round(customServiceDuration) : null, custom_service_price:isCustomService ? Math.round(customServicePrice) : null, start_time: new Date(start).toISOString(), status: 'pending', custom_items: customItems.map(({ name, amount }) => ({ name: name.trim(), amount })) }); close(); } catch (error) { setMessage(appointmentMutationErrorMessage(error)); } finally { setSaving(false); } };
  return <Dialog.Root open={open} onOpenChange={value => { if (!value) close(); }}><Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-[200] bg-black/60"/><Dialog.Content className="checkout-dialog fixed left-1/2 top-1/2 z-[201] max-h-[92vh] w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto p-5"><div className="flex justify-between"><div><Dialog.Title className="text-2xl font-black">新增預約</Dialog.Title><Dialog.Description>可引用已建立的服務，或只為本次預約自訂服務</Dialog.Description></div><Dialog.Close className="min-h-12 min-w-12"><X className="mx-auto"/></Dialog.Close></div><form className="mt-5 grid gap-3" onSubmit={save}><label>顧客<select required value={customerId || customers[0]?.id || ''} onChange={event => setCustomerId(Number(event.target.value))}>{customers.map(customer => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><label>服務<select required value={selectedServiceId} onChange={event => setServiceId(Number(event.target.value))}>{activeServices.map(service => <option key={service.id} value={service.id}>{service.name} · {service.duration_minutes} 分 · NT$ {service.price}</option>)}<option value={-1}>＋ 自訂本次服務</option></select></label>{isCustomService && <div className="grid gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3"><label>自訂服務名稱<input required value={customServiceName} onChange={event => setCustomServiceName(event.target.value)} placeholder="例如：局部燙髮"/></label><label>所需時間（分鐘）<input required type="number" min="1" step="5" value={customServiceDuration} onChange={event => setCustomServiceDuration(Number(event.target.value))}/></label><label>服務價位（NT$）<input required type="number" min="0" step="1" value={customServicePrice} onChange={event => setCustomServicePrice(Number(event.target.value))}/></label></div>}<label>開始<input required type="datetime-local" value={start} onChange={event => setStart(event.target.value)}/></label><CustomLineItemEditor items={customItems} onChange={setCustomItems}/>{message && <p className="settings-error" role="alert">{message}</p>}<button disabled={saving || !customers.length} className="min-h-12 rounded-xl border border-amber-400/40 bg-amber-500 font-semibold text-slate-950 transition-colors hover:bg-amber-400 disabled:border-slate-700 disabled:bg-slate-800 disabled:text-slate-500">{saving ? '儲存中…' : '建立待確認預約'}</button></form></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
